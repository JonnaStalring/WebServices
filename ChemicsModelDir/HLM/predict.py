...
def getPrediction(smi, modelDirPath):
    ...
    REGMODEL = os.path.join(modelDirPath, "regression/RFmodel_regression")
    CLASSDATA = os.path.join(modelDirPath, "classification/trainData.txt")

    predictor = AZOrangePredictor.AZOrangePredictor(REGMODEL)
    predictor.getDescriptors(smi)
    predReg = predictor.predict()

    predClass = getCP(CLASSDATA, predictor.smilesData)

    if predClass == "Both" or predClass == "Empty":
        conf = "Low"
    elif predClass == "Low" and predReg > 45:
        conf = "Low"
    elif predClass == "High" and predReg < 45:
        conf = "Low"
    else: 
        conf = "High"
